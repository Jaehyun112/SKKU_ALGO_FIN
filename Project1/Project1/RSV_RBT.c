...
bool RSV_RB_INSERT(RSV_node_ptr * Root, char *KEY) {
...
// DELETE =================================================
RSV_node_ptr RB_DELETE(RSV_node_ptr *root, char *KEY) {
	// 1) Search
	RSV_node_ptr delNode = TREE_SEARCH(*root, KEY);
	RSV_node_ptr y; // 대체노드
...
// ===================================================================
RSV_node_ptr TREE_SEARCH(RSV_node_ptr root, int KEY) {
	if (root == Sentinel)
		return Sentinel;
	if (root->Key == KEY)
		return root;
